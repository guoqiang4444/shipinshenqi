...
package com.lzc.pineapple.cache.manager;
...
import com.lzc.pineapple.cache.provider.DownloadEntity;
import com.lzc.pineapple.cache.provider.Downloads;
...
public class DownloadManager {
	...
	public final static String COLUMNN_COVER = "cover";

	public final static String COLUMN_SPEED_BYTES = "speed_bytes";
	...
	private static final String[] COLUMNS = new String[] { COLUMN_ID,
			...
			COLUMN_REASON, COLUMN_BYTES_DOWNLOADED_SO_FAR, COLUMN_SPEED_BYTES,
			COLUMN_LAST_MODIFIED_TIMESTAMP, COLUMNN_COVER, };
	...
	private static final String[] UNDERLYING_COLUMNS = new String[] {
			...
			Downloads.COLUMN_SPEED_BYTES, Downloads.COLUMN_LAST_MODIFICATION,
			Downloads.COLUMN_DESTINATION, Downloads.COLUMN_FILE_NAME_HINT,
			Downloads._DATA, Downloads.COLUMN_COVER, };

	private static final Set<String> LONG_COLUMNS = new HashSet<String>(
			Arrays.asList(COLUMN_ID, COLUMN_TOTAL_SIZE_BYTES, COLUMN_STATUS,
					COLUMN_REASON, COLUMN_BYTES_DOWNLOADED_SO_FAR,
					COLUMN_SPEED_BYTES, COLUMN_LAST_MODIFIED_TIMESTAMP));
	...
	public static class Request {
		...
		private Uri mUri;
		private CacheVideoInfo cacheVideoInfo;
		...
		public Request(CacheVideoInfo cacheVideoInfo) {
			String url = cacheVideoInfo.getUrl();
//			url = "http://218.108.232.138/videos/v0/20140919/c3/86/0a8af071a7dc1099689499b9207b1ac0.f4v?z=hangzhouhuashu_cnc&key=bbfe8aec2ffbd694&uuid=3a6403a4-5420369b-39";
			url  = URLDecoder.decode(url);
			initUri(url);
			this.cacheVideoInfo = cacheVideoInfo;
		}
        private void initUri(final String url){
        	new Thread(new Runnable() {
				...
				public void run() {
					...
					Log.d("lzc","url before handle===>"+url);
					String realUrl = getRealVideoUrl(url);
					Log.d("lzc","url after handle===>"+url);
		...
		ContentValues toContentValues(String packageName) {
			...
			values.put(Downloads.COLUMN_COVER, cacheVideoInfo.getCover());
			...
			putIfNonNull(values, Downloads.COLUMN_TITLE,
					cacheVideoInfo.getName());
	...
	public static class Query {
		...
		private boolean isShowCompletedSuccessful = true;
		...
		public Query setShowCompletedSuccessful(
				boolean isShowCompletedSuccessful) {
			this.isShowCompletedSuccessful = isShowCompletedSuccessful;
			return this;
		}
		...
		Cursor runQuery(ContentResolver resolver, String[] projection,
			...
			if (!isShowCompletedSuccessful) {
				// Downloads.STATUS_SUCCESS = 200
				selectionParts.add(Downloads.COLUMN_STATUS + " != '200'");
			}
	...
	public void pauseAllDownload() {
		Query baseQuery = new Query().setOnlyIncludeVisibleInDownloadsUi(true);
		baseQuery.setShowCompletedSuccessful(false);
		Cursor cursor = query(baseQuery);
		try {
			for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor
					.moveToNext()) {
				int status = cursor
	...
	public void resumeAllDownload() {
		...
		try {
			for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor
				...
				int status = cursor
						.getInt(cursor.getColumnIndex(COLUMN_STATUS));
				if (status == STATUS_FAILED || status == STATUS_PAUSED) {
					int mIdColumnId = cursor
							.getColumnIndexOrThrow(DownloadManager.COLUMN_ID);
					long id = cursor.getInt(mIdColumnId);
					resumeDownload(id);
	...
	public void restartDownload(long... ids) {
		...
		values.put(Downloads.COLUMN_SPEED_BYTES, 0);
	...
	private static class CursorTranslator extends CursorWrapper {
		...
		private String translateString(String column) {
			...
			if (column.equals(COLUMNN_COVER)) {
				return getUnderlyingString(Downloads.COLUMN_COVER);
			}
		...
		private long translateLong(String column) {
			...
			if (column.equals(COLUMN_SPEED_BYTES)) {
				return getUnderlyingLong(Downloads.COLUMN_SPEED_BYTES);
			}
